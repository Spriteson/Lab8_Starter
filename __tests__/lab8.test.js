describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    console.log('Button text after clicking:', buttonText);
  }, 2500);
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    // Assertion to check if the cart count is 20
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    // Check every element to ensure their buttons say "Remove from Cart"
    for (const productItem of productItems) {
      // Get the shadowRoot and query select the button inside
      const shadowRoot = await productItem.getProperty('shadowRoot');
      const button = await shadowRoot.$('button');
      
      // Check if the button's innerText is equal to "Remove from Cart"
